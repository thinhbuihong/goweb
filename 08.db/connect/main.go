...
func insert(w http.ResponseWriter, req *http.Request) {
	...
	stmt, err := db.Prepare(`INSERT INTO customer (name) VALUES ($1);`)
	check(err)
	defer stmt.Close()

	r, err := stmt.Exec("thinh")
	check(err)
...
func update(w http.ResponseWriter, req *http.Request) {
	stmt, err := db.Prepare(`UPDATE customer SET name=$1 WHERE name=$2;`)
	check(err)
	defer stmt.Close()

	r, err := stmt.Exec("Jimy", "thinh")
	check(err)
...
func del(w http.ResponseWriter, req *http.Request) {
	stmt, err := db.Prepare(`DELETE FROM customer WHERE name=$1;`)
	check(err)
	defer stmt.Close()

	r, err := stmt.Exec("Jimy")
	check(err)
